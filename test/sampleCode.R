...
rMeanPar <- lgi.parRapply(d, mean, njobs=3, join.method=c)
all(rMean == rMeanPar)
cMean    <- apply(d,2,mean)
cMeanPar <- lgi.parCapply(d, mean, njobs=3, join.method=c)
all(cMean == cMeanPar)
#
# returns a vector and has a function argument
#
rMean2    <- apply(d,1,mean,trim=0.02)
rMean2Par <- lgi.parRapply(d, mean, trim=0.02, njobs=3, join.method=c)
all(rMean2 == rMean2Par)
cMean2    <- apply(d,2,mean,trim=0.2)
cMean2Par <- lgi.parCapply(d, mean, trim=0.2,  njobs=3, join.method=c)
all(cMean2 == cMean2Par)
#
# returns a matrix
#
dLog     <-  apply(d,1,log)
dLogPar  <-  lgi.parRapply(d, log, njobs=5, join.method=cbind)
all(dLog == dLogPar)
#
...
dFitRPar <- lgi.parRapply(d, func1,y=response, njobs = 3, join.method=c)
dFitC    <- apply(d,2,func1,y=response[1:nrows])
dFitCPar <- lgi.parCapply(d,func1,y=response[1:nrows], njobs = 5, join.method=c)
...
result2  <- lgi.parLapply( dFitRPar,func2, njobs = 3)
ra1 <- array()
...
resultC2 <- lgi.parLapply(dFitC,func2, njobs = 4)
ra2 <- array()
